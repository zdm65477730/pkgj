...
export VITASDK=$(pipenv run conan info vitasdk-toolchain/1199@blastrock/pkgj --paths -pr vita | grep -Po '(?<=package_folder: ).*$')
export CC=gcc-7
export CXX=g++-7
...
# cmake -GNinja ../.. -DCMAKE_BUILD_TYPE=RelWithDebInfo -DHOST_BUILD=ON -DPKGI_ENABLE_SERIAL_LOGGING=ON
# ninja
# cd ..
